...
//Excel中的Type支持int、float、bool、string以及对应的数组类型

func Load(excelMap map[string]string, outPath string) {
	entityHeader := "using System.Collections;\nusing System.Collections.Generic;\n\n"
	entityResult := ""
	...
	utils.SaveData(entityHeader+entityResult, outPath+"DataEntity.cs")
}
...
func readExcel(path string) (map[string]map[string]map[string]interface{}, string) {
	...
	if err == nil {
		...
		for _, sheetName := range sheetMap {
			...
			entityStr += "public class " + sheetKey + "  \n{\n"
			if len(keyArr) == len(typeArr) {
				for k, v := range typeArr {
					if v == "none" || v == "" {
						continue
					}
					entityStr += "    public " + typeArr[k] + " " + keyArr[k] + ";\n"
				}
			}
			entityStr += "}\n\n"
		}
...
func getValueByType(str string, typeStr string) interface{} {
	...
	} else if strings.Contains(typeStr, "int") || strings.Contains(typeStr, "float") {
		if strings.Contains(typeStr, "[]") {
			for _, v := range strArr {
